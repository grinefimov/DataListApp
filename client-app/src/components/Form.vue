<template>
  <div class="col-xl-4 offset-xl-4 col-md-6 offset-md-3">
    <b-card :title="'New ' + this.$root.$data.context.dataName">
        <b-form @submit.prevent="createData" @change="creatingNewData">
          <b-form-group :label="this.$root.$data.context.dataNames[0]">
            ...
            <b-form-invalid-feedback :state="data1Validation">
              {{ this.$root.$data.context.dataNames[0] }} must be 2-20 characters long.
          ...
          <b-form-group :label="this.$root.$data.context.dataNames[1]">
            <b-form-select v-model="model.data2" :options="this.$root.$data.context.data2Options" required>
          ...
          <b-form-group :label="this.$root.$data.context.dataNames[3]">
            <b-form-radio-group v-model="model.data4" required :options="this.$root.$data.context.data4Options"></b-form-radio-group>
          </b-form-group>
          <b-form-group :label="this.$root.$data.context.dataNames[4]">
            ...
            >
              {{ this.$root.$data.context.data5Text }}
            </b-form-checkbox>
          </b-form-group>
          <div>
      ...
      <b-alert :show="dataSaved" variant="success" class="mt-2">{{ this.$root.$data.context.dataName }} saved</b-alert>
  </div>
</template>
